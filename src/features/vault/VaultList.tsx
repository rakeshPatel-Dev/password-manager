import { ChevronDown, Eye, EyeOff, FolderInput, LinkIcon, Pencil, Trash2 } from 'lucide-react'
...
export function VaultList({
  ...
  const toggleCollapsed = useCallback((entryId: number) => {
    setCollapsedMap((prev) => ({ ...prev, [entryId]: !prev[entryId] }))
  }, [])
  ...
  return (
    <div className="space-y-4" role="feed" aria-label="Vault entries">
      {filtered.map((entry) => {
        ...
        const isCollapsed = collapsedMap[entry.id] ?? false
